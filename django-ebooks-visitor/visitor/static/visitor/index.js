...
app.controller('DisplayController', function ($scope, $http) {
	...
	$scope.updateDisplay = function (pageSerial, searchWords) {
		...
		$http.get("../visitor/page?pageSerial=" + pageSerial).
			success(function (response) { //data, status, headers, config
				var result;
				if ($scope.searchOption == "or-fix" || $scope.searchOption == "and-fix") {
					result = utils.highlightSubstring(response, searchWords);
				} else if($scope.searchOption == "statement") {
					result = utils.highlightStatement(response, searchWords);
				} else {
					result = utils.highlight(response, searchWords);
				}
				$scope.processDisplayText(result, pageSerial);
			});
	};

	$scope.updateDisplayFromIndex = function (pageSerial, indexHighlightCode) {
		if (pageSerial === undefined) {
			pageSerial = 1;
		}
		$http.get("../visitor/page?pageSerial=" + pageSerial + "&indexHighlightCode=" + indexHighlightCode).
			success(function (response) { //data, status, headers, config
				var result = response;
				$scope.processDisplayText(result, pageSerial);
			});
	...
	$scope.processDisplayText = function(displayText, pageSerial) {
				var result = utils.processNewLines(displayText);
	...
	//This one is called slowly from Mogama and headless TOC
	...
	$scope.displayFromTocSearch = function (hit) {
		if(hit.serial === "None") {
			$scope.updateTOC(hit.topicid);
			$scope.showToc();
			return;
		}
		$scope.updateDisplay(hit.serial);
		$scope.updateTOCBySerial(hit.serial);
		$("#right-panel").panel("close");
	};

	$scope.displayFromIndexHits = function (pageSerial, highlightIndexCode) {
		$("#left-panel").panel("close");
		$scope.updateDisplayFromIndex(pageSerial, highlightIndexCode);
		$scope.updateTOCBySerial(pageSerial)
	};
...
//TOC Management
	$scope.updateTOC = function (curTopicId) {
		$http.get("../visitor/topics?topic=" + curTopicId).
			success(function (response) { //data, status, headers, config
	...
	$scope.updateTOCBySerial = function (serialNo) {
		$http.get("../visitor/topics?serial=" + serialNo).
			success(function (response) { //data, status, headers, config
				//$scope.tocCurTopicId = curTopicId;
				$scope.tocTree = response;
				setTocHitsHeight();
	...
	$scope.updateTocSearch = function (tocSearchWords) {
		//if (tocSearchWords.length == 0) {
		//	$scope.showToc();
		//} else {
		if (tocSearchWords.length > 0) {
			//$scope.showTocSearch();
			$http.get("../visitor/searchToc?word=" + tocSearchWords).
				success(function (response) {
					//$scope.tocTree = response;
					//setTocHitsHeight();
					$scope.tocSearchHits = response;
	...
	$scope.showTocSearch = function (tocSearchWords) {
			$("#toc-tree").hide();
			$("#toc-search-list").show();
	};

	$scope.showToc = function (tocSearchWords) {
			$("#toc-tree").show();
			$("#toc-search-list").hide();
	};

//Indexes Management
	$scope.loadIndexHits = function (indexType, arrayIndex) {
		$http.get("../visitor/fehres?type=" + indexType).
			success(function (response) { //data, status, headers, config
	...
	$scope.updateIndexItemHits = function (indexType, id) {
		$http.get("../visitor/fehresPages?type=" + indexType + "&id=" + id).
			success(function (response) { //data, status, headers, config
...
//Search Management
	$scope.updateSearchHits = function () {
		...
		var url = "../visitor/search?word=" + $scope.searchWords + "&page=" + $scope.searchCurPageNo +
...
//Bookmarks
	...
	$scope.linkToArticle = function (id) {
		$http.get("../visitor/serial?topicId=" + id).
			success(function (result) {
				$scope.updateDisplay(result.serial);
			});
	};

	$scope.displayDefinition = function (type, id) {
		//TermALAM-id-32+54+55+319-join-1065
		//definitions could be multiple pages, following the format: 32+54+55+319
		//+ is a reserved char in http, so I will replace by comma
		var ids = id.replace(/\+/g, ",")
		$http.get("../visitor/definition?type=" + type + "&id=" + ids).
			success(function (result) {
				$.mobile.changePage("#popup-dialog-def", {changeHash: false});
				$scope.definitions = result;
				$scope.showDefinition(0);
			});
	};

	$scope.showDefinition = function (index) {
		for(var i = 0; i< $scope.definitions.length; i++) {
 			var element = "#def" + i;
			if(i == index) {
			  //$(element).show();
				//I am addClass/removeClass as as I use ng-hide directive in html
	...
	$scope.updateMa3lama = function (id, pageNo) {
		...
		$http.get("../visitor/ma3lama_books?book=" + id + "&page=" + pageNo).
			success(function (result) {
				result.text = result.text.replace(/href="([^"]+)"/g,
					'href onclick="onHandleDisplayLinkClick(\'$1\')"');
...
function setIndexHitsHeights() {
	var indexHeight = $(window).height() - $("#tabs-header-top").height() -
...
function setBookmarksListHeight() {
	var height = $(window).height() - $("#tabs-header-top").height();
	if ($(window).width() >= 800) {
		height = height - $(".demo-page-top-header").height() - 10;
	}
